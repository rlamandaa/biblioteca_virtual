...
class Biblioteca {
  ...
  void removerLivro(String criterio) {
    ...
    int removidos = livros.where((livro) => livro.titulo == criterio || livro.autor == criterio || livro.id == criterio).length;
    
    // Removendo os livros que correspondem ao critério
    livros.removeWhere((livro) => livro.titulo == criterio || livro.autor == criterio || livro.id == criterio);
...
void main() {
  ...
  biblioteca.cadastrarLivro(Livro('1', 'Psicologia das Cores', 'Eva  Heller', 2013, 'Ciências Sociais'));
  biblioteca.cadastrarLivro(Livro('2', 'Reading Images: the grammar of visual desing', 'Gunther Kress and Theo van Leeuwen', 2006, 'Gramática'));
  biblioteca.cadastrarLivro(Livro('3', 'Dom Casmurro', 'Machado de Assis', 1899, 'Romance'));
  biblioteca.cadastrarLivro(Livro('4', 'Abusado: o dono do morro Dona Marta', 'Caco Barcellos', 2003, 'Jornalismo literário'));
